...
<div class="container-fluid">
    ...
    $access = check($uid,['/manage/rolefunction/edit','/manage/rolefunction/delete','/manage/rolefunction/add']);
     ?>
                ...
                <div class="modal-body">
                    <form class="form-horizontal" role="form" id="form1" method="post" action="<?php echo url('add'); ?>">
                        <div class="form-group">
                            <label for="role_code" class="col-sm-2 control-label">权限组名称</label>
                            <div class="col-sm-10">
                        ...
                        <div class="form-group">
                            <label for="function_code" class="col-sm-2 control-label">角色代码</label>
                            <div class="col-sm-10">
                                <div class="checkbox">
                                    <?php if(is_array($allgroup) || $allgroup instanceof \think\Collection || $allgroup instanceof \think\Paginator): $i = 0; $__LIST__ = $allgroup;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$a): $mod = ($i % 2 );++$i;?>
                                    <label class="checkbox-inline">
                                        <input type="checkbox" class="function_code" value="<?php echo $a['id']; ?>" name="function_code[]"> <?php echo $a['name']; ?>
                                    </label>
                                    <?php endforeach; endif; else: echo "" ;endif; ?>
                                </div>
                            </div>
        ...
        switch (type){
            ...
            case 'edit':
                ...
                $.get("<?php echo url('edit'); ?>?do=get&rid="+id,function (data) {
                    ...
                    $('#role_code').val(data.info.rolecode)
                    $('#selfcode').val(data.info.id)
                    if ($('.function_code').each('checked')) {
                        // do something
                        alert($('.function_code').val())
                    }
                });
